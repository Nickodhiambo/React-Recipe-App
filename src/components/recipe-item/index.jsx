...
export default function RecipeItem({ item }) {
    return <div
        ...
        <div>
            ...
            <h3 className="text-bold text-2xl truncate text-black">{item?.title}</h3>
            <Link to={`/recipe-item/${item?.id}`}
                className="text-sm p-3 px-8 mt-5 rounded-lg font-medium tracking-wider inline-block shadow-md text-white bg-black">
                Recipe Details
            </Link>
        </div>
    </div>
